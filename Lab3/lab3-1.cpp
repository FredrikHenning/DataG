...
int main(int argc, char const *argv[])
{
  ...
  for (int i = 0; i < 20; i++) {
      ...
      glm::vec4 normal;
      normal = glm::vec4(glm::cross(P2MP1, P3MP1), 0.0);
  ...
  // Generateing OpenGL texture object
  ...
  unsigned image_w;
  unsigned image_h;
  unsigned err_code = lodepng_decode32_file(&image_data, &image_w, &image_h, "../../common/data/numberline_hires.png");
  if (!err_code)
  ...
  while (!glfwWindowShouldClose(window))
  {
      ...
      glUniformMatrix4fv(modViewMat_location, 1, GL_FALSE, glm::value_ptr(modelMatrix));
      // -----------------------------------------------------------------------//
